...
万块的板。所以它不是玩具 demo，是扎在真实流程上、有 **715 条测试 + GitHub Actions
CI** 兜底的工程。
...
- 🧩 **技术亮点：** tool-calling agent loop（12 个工具）· Refdes Guard 防幻觉 · Evidence Ledger 证据账本 · prompt caching · 分层模型路由 · 715 测试 + CI
...
Copilot 解释呈现证据化评审队列，配套 715 条回归测试。
