...
> 面向公开硬件项目的设计验证工作台：位号来自 EDA registry，检视意见必须带证据 token，Agent 只能通过结构化工具追问原理图。
...
Phase 4 的当前口径是“一条 trust architecture，两条 public input tracks”，不是假装同一块公开板覆盖所有命令：

```text
Refdes Guard + Evidence Ledger + L1 deterministic validators + structured tools
```

C3/C4 的 coverage loop 是支撑材料：C3 给 profile gap 排序，C4 把选中的 L3/manual group 推进到 L1 deterministic rows。它证明这条产品闭环可重复，但主角仍然是 trust：模型被 registry object、evidence token 和 tool returns 约束住。
