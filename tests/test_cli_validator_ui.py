...
def test_design_validator_ui_auto_matches_profiles_and_writes_index(
    ...
    assert "validated=15" in result.output
    assert "PASS/WARN/ERROR=5/8/2" in result.output
    ...
    assert "| Validated components | 15 |" in index_text
    assert "| PASS / WARN / ERROR | 5 / 8 / 2 |" in index_text
...
def test_design_validator_ui_auto_matches_controller_power_stage(
    ...
    assert result.exit_code == 0, result.output
    assert "design-validator-ui:" in result.output
    assert "validated=19" in result.output
    assert "PASS/WARN/ERROR=5/11/3" in result.output
    ...
    index_text = index_output.read_text(encoding="utf-8")
    assert "| Validated components | 19 |" in index_text
    assert "| PASS / WARN / ERROR | 5 / 11 / 3 |" in index_text
