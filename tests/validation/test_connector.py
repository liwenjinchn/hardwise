...
"""
    ...
    signal_4_checks = [c for c in results.component_checks if c.check == "connector_signal_4_connectivity"]
...
"""
    ...
    assert results.status == "ERROR"
    ground_checks = [c for c in results.component_checks if c.check == "connector_ground_connectivity"]
    assert len(ground_checks) == 1
    assert ground_checks[0].status == "ERROR"
    assert "not a recognized ground net" in ground_checks[0].summary
