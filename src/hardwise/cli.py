...
def design_validator_ui(
    ...
    if context.document_report is not None:
        ...
        typer.echo(
            ...
            f"(datasheets_matched={doc_counts['matched']}, no_result={doc_counts['no_result']}, "
    ...
    typer.echo(
        ...
        f"BOM rows matched={context.index.bom_matched}, "
...
def serve_workbench(
    ...
    if context.document_report is not None:
        doc_counts = context.document_report.counts_by_status
        document_state = (
            f"on datasheets_matched={doc_counts['matched']}, no_result={doc_counts['no_result']}, "
            f"ambiguous={doc_counts['ambiguous']}, manual_needed={doc_counts['manual_needed']}"
        )
...
def eval_pack(
    ...
    if static_snapshot and (baseline is not None or accept_baseline):
        typer.echo("error: --static-snapshot does not support --baseline/--accept-baseline", err=True)
        raise typer.Exit(1)
