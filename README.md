...
If you only have 90 seconds, start with the offline Copilot workbench generated
by `design-validator-ui --ai-snapshot`:
...
offline Copilot workbench generated by `design-validator-ui --ai-snapshot`.
...
The 90-second main stage is the exported-netlist workbench:
...
That path consumes an exported schematic netlist/PST plus BOM, auto-matches
public datasheet profiles by BOM identity, and writes a single static HTML
...
| Copilot trace | Expand a baked Copilot Evidence / Tool trace. | The agent explains structured tool results; it does not decide PASS/WARN/ERROR. |
...
`design-validator-ui --ai-snapshot` bakes audited offline chat transcripts into
the single HTML file (no server, no API key); `serve-workbench` runs a local
FastAPI server whose `--fake-ai` mode drives the real agent loop with a
deterministic fake client, and whose real mode talks to any Anthropic-format
endpoint configured in `.env`. Every panel answer runs the same five-tool
Runner and the same Refdes Guard, so an unknown refdes such as `U999` is wrapped
as `⟨?U999⟩` rather than fabricated.
