...
| 2 | 主导硬件效能工具及平台的设计与开发（如开发/测试工具、数字化作业平台、作业场景 AI Agent 等），提升硬件作业效率、质量 | headline 是“五大机制 + L1/L2/L3 trust 分层”这条 trust architecture，下面的数字都是它的证据：KiCad `pic_programmer` 轨一行 `hardwise review --rules R001,R002,R003,DS001 --report-style component` 产 29 findings / 121 components，并在 U3/L78 上引用 `datasheet:l78.pdf#p4`；L78 是唯一一条 `ingest-datasheet -> query-datasheet -> ask --vector` live smoke（其余 profile 为 reviewed evidence，边界见 `docs/evidence_chain_audit.md`），证明 agent 可以先 `search_datasheet` 再引用 `l78.pdf p4`；agent 工具层已有 `run_component_validation(refdes)`，测试证明 Runner 能驱动 deterministic validator；Allegro `design-validator-ui --ai-snapshot` 轨产 25 components / 22 L1 rows / PASS-WARN-ERROR=5/13/4 离线 Copilot 工作台，其中 9 个 profile-backed targets + 13 个 generic passive checks | ✅ Phase 4 demo closeout |
