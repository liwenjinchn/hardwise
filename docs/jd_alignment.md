...
| 2 | 主导硬件效能工具及平台的设计与开发（如开发/测试工具、数字化作业平台、作业场景 AI Agent 等），提升硬件作业效率、质量 | CLI `hardwise review` 命令一行触发 → 84 个 findings + SQLite store + 候选规则 + markdown 报告对齐 SCH_review_feedback_list；`data/checklists/sch_review.yaml` 3 条 active 规则 (R001/R002/R003) + 2 条 planned (R004/R005) | ✅ R001-R003 ＋ ⏳ R004/R005 (slice 4-5) |
...
| 4 | 构建并维护向量数据库（如 Milvus、Qdrant、Chroma 等），及结构化数据库（如 PostgreSQL、MySQL） | `src/hardwise/store/vector.py` (Chroma local persistent + ONNX MiniLM, l78.pdf 157 chunks) + `src/hardwise/store/relational.py` (SQLite + SQLAlchemy 2.0, components + nc_pins 表) + `src/hardwise/ingest/pdf.py` (pdfplumber 切页 + 滑窗分块)；JD 里 PostgreSQL/Chroma 是同类型代表，MVP 用 SQLite + Chroma 体现同一架构模式 | ✅ |
| 5 | 推动 AI Agent、自动化工具、数字化作业流在真实硬件项目中的落地与闭环迭代，持续跟踪使用数据，优化准确性与作业流顺畅度 | 3 条 active 规则在公开 KiCad 工程 `pic_programmer` 上闭环；`src/hardwise/memory/consolidator.py` Sleep Consolidator 候选规则池 (R002/R003 medium 两条已沉淀) + 人工 gate；`docs/learning_log.md` 跟踪每个 slice 的命中/漏检/误报（含 KiCad pin.at 锚点规约、Chroma 默认 embedder 等 4 条调试条目） | ✅ R001-R003 ＋ ⏳ R004/R005 (slice 4-5) |
...
| 5 | 了解向量数据库（如 Milvus、Qdrant）和结构化数据库的实际建库与使用经验 | `store/vector.py` (Chroma) + `store/relational.py` (SQLite) + `ingest/pdf.py`；面试讲"为什么 datasheet 入向量库 / refdes 入结构化库 / refdes 是 join key"+ "Chroma → Qdrant 是 config 改动而非重写 + SQLite → PostgreSQL 是 SQLAlchemy URL 改动" | ✅ |
