...
1. **Two parallel pipelines that do not connect.** Pipeline A is `hardwise review <kicad>` — checklist rules (R001/R002/R003/DS001) plus the Anthropic-format tool-use loop in `agent/runner.py`. At the time of this audit it exposed four query tools (`list_components`, `get_component`, `get_nc_pins`, `search_datasheet`); the current runtime surface is 5 core query tools plus 7 workbench context/topology tools. Pipeline B is the `design-validator-ui` / `report-validator-ui` workbench — the deterministic `validation/` family validators (buck, gate_driver, mcu, i2c_mux, diode, connector, mosfet). The audit gap was that the strongest deterministic work (the family validators) was unreachable from the agent; the bridge later landed as `run_component_validation(refdes)`.
...
- 2026-06-05 — DR-014 submission trust-story closeout (3 of 4 items): the d3a/d3b integration line was merged to public `main` via PR #1 (`2f85b10`). Item 2 (evidence-chain re-audit, `67b03c8`): `docs/evidence_chain_audit.md` re-audited after the merge to a full 25-profile ledger — exactly 1 real-PDF-backed + live-retrieval-smoked (`l78.json`), 22 reviewed-token-without-local-PDF, 2 non-PDF-scheme (`1_5smc15a`, `ltst-c190kgkt`); the shared `datasheet:<file>#pN` shape between `ingest/pdf.py` output and hand-authored tokens is flagged as the structural over-claim hazard. Item 3 (narrative reset, `84de445`): README / interview_qa / interview_narrative / jd_alignment reordered so the headline is the trust architecture (Refdes Guard + Evidence Ledger + L1 deterministic + L2/L3 boundary) and coverage counts (4010 / 8180 / 195) follow the "支撑材料" framing; zero numbers dropped, pure reorder. Items 2+3 shipped together as PR #2. Item 4 (thin C5 L2 trust slice): assessment found the L2 grounded path already implemented and tested — (a) `grounding.py:trust_tier_for_datasheet_search` promotes to L2 only on page-tokened hits, (b) no-hit / no-collection downgrades to L3 (`runner.py:225,238`, tested), (c) L2-never-overrides-L1 held structurally but was untested. `test_runner_l2_search_does_not_override_l1_validation` added to `tests/agent/test_validation_bridge.py`: one run produces both an L1 `run_component_validation` trace (validated/PASS) and an L2 `search_datasheet` trace, asserting they stay independent and the L1 verdict is not mutated. Sentence-level prose spec-claim gating is deliberately deferred as a heavier future slice (would need a new DR). `uv run pytest -q` → 533 passed, 7 deselected; `uv run ruff check .` → clean.
