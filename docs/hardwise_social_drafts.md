...
当前公开 demo 能生成 25 components / 17 validated rows / PASS-WARN-ERROR=5/9/3 / 8 manual rows 的 workbench。未知 refdes 会被包成 `⟨?U999⟩`，L78 能展示 `ingest -> retrieve -> agent citation` 的 page-level evidence。项目从命令行 demo 收成了可用于面试和公开展示的硬件 AI 工作台。
