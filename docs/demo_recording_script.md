...
1. 项目摘要，10 秒
   ...
   指顶部指标：25 个器件、22 行 L1 验证、通过-警告-错误=5/13/4、3 行人工缺口。说：Hardwise 消费导出的网表/PST + BOM，生成 Layout 前评审工作台——这种规模的板子，评审会前人工对位号、翻规格书通常以小时计，这里导入后约一分钟分出三类队列。这不是实时 EDA 插件，也不是让模型自由审板。
...
3. `U12` 确定性错误，18 秒
   ...
   点 `U12`，展示 XL1509 降压错误。说：这个错误不是模型意见，是确定性验证器从网表和 XL1509 器件档案得到的结论：`L1=6.8uH` 低于档案最小值，`D5=1N4007W` 不是肖特基类续流二极管——1N4007 是慢恢复整流管，在 XL1509 约 150kHz 的开关频率下反向恢复损耗大，这正是评审会上人眼最容易放过的一类错。
...
6. L78 证据页码，12 秒
   ...
   点 L78 证据问题或 U1/L7805 轨迹，停在 `datasheet:l78.pdf#p4`。说：规格书 PDF 切块进 Chroma 向量库，按位号 + 页码挂 metadata，检索结果自带页码出处。L78 这条是 L2 有出处证据，供评审人核对，但它不会自动升级成模型自由判断。
