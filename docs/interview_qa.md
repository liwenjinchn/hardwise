...
**v5.0 (V3.5 validation index UX)**: 单器件 PCA9548A 和第二个 regulator template 跑通后，项目级 validation index 不再把 4000 多个 no-profile row 直接铺满 markdown，而是把 no-profile rows 按 BOM identity 聚合成 `Profile Candidate Summary`，并用 refdes / identity hint 区分 active 与 passive candidate。`validate-allegro-project` 新增 `--manual-limit` 和 `--candidate-limit`：markdown 默认给 reviewer 看 validated components、active no-profile candidates 和有限 manual rows；JSON sidecar 仍保留全部 `rows`、`candidate_groups`、`active_candidate_groups`，供后续 Web UI / component workspace 使用。公开 switch-board 样本 smoke 仍是 5 个 PCA9548A validated、25 个 PASS，新增的是“下一步该补哪个 profile/validator”的 triage 入口，而不是让模型自由猜剩余 4005 个器件。

**v5.1 (V3.6 real candidate expansion)**: V3.6 没批量猜 catalog，而是从 V3.5 的 active candidate list 里挑一个真实高复用器件 `PCA9617ADP`，按 NXP 公开 datasheet 固化成 `PCA9617A` profile，并新增 `pca9617a` validator。规则仍是 deterministic：VCCA/VCCB 只接受可解析的命名电源网并对比推荐范围，GND 必须接地，Port A/B 的 SCL/SDA 必须存在且连接，EN 必须连接。真实 switch-board 样本 smoke 从 5 个 PCA9548A validated / 25 PASS 扩到 13 个 validated / 73 PASS，其中新增 8 个 PCA9617ADP；剩余 3997 个仍保持 `no_profile`，不会被模型自由补结论。

**v5.2 (V3.7 MOSFET profile expansion)**: V3.7 继续从 active candidate list 里挑最大真实 no-profile group：`LN2312LT1G` N-MOS。按公开 LRC/JLC 信息和真实 PST pin mapping 固化 `LN2312LT1G` profile，并新增通用 `nmos` validator：只验证 G/S/D pin 存在、三端已连接、gate 已连接，以及当 D/S 或 G/S 两端 net name 都能解析成明确 rail voltage 时才比较 VDS/VGS。公开资料摘要能确认 `VDS=20V`、`ID=4.9A`、`Pd=750mW`，但未可靠确认 VGS abs max，所以 profile 不写 `abs_max.vgs`，VGS 检查返回 `manual_needed`。真实 switch-board smoke 从 13 个 validated / 73 PASS 扩到 69 个 validated / 241 PASS / 112 manual_needed；这些 manual_needed 主要来自信号网电压不可由 net name 证明，体现的是“不猜电压”的可靠性边界。

**v5.3 (V3.8 shift-register profile expansion)**: V3.8 继续沿着 active candidate list 补真实高复用器件 `74LV165PW`。按公开 74LV165 datasheet 固化 `74LV165` profile，并新增 `74lv165` validator：VCC 只根据明确 rail net name 对比推荐 1.0V-5.5V，GND 必须接地，D0-D7、`/PL`、CP、`/CE`、DS、Q7 必须存在且连接，`/Q7` 可以接 net 或明确 NC。这里不判断 SGPIO 链路时序、级联方向、输入语义或上拉下拉是否合理，因为这些需要设计意图和更完整规则。真实 switch-board smoke 从 69 个 validated / 241 PASS 扩到 79 个 validated / 301 PASS / 112 manual_needed；新增 10 个 `74LV165PW` 每个 6 PASS / 0 manual_needed，且每条检查都有 BOM、datasheet、design 或 net-name rule token。
...
**v4.9 input/output boundary**: V2.9 之后输入模型可以这样讲：native KiCad project 是最高保真入口，有位号、symbol 字段、datasheet path 和可视定位；Allegro schematic netlist 是跨 EDA 的 topology 入口，覆盖 Telesis 单文件和 Capture/Allegro PST 三件套，有 refdes/pin/net；schematic-exported BOM 是 netlist-only 场景的 component identity aid，用 refdes 补 value/MPN/manufacturer，不是 PLM BOM；本地 document index 是 datasheet/document link aid，用 BOM identity 匹配公开文档，不是 live supplier database。输出上，KiCad `review` 产 checklist finding report；Allegro `report-allegro-bom` 产 component-centric intake/index report：默认完整报告保留全 component table，`--summary-only` 给 reviewer 先扫前缀统计、BOM item groups 和可选 document match，`--mismatch-only` 专门用于快速看 BOM/design join 问题且不接受 `--document-index`。Hardwise 当前 shipped 的是 KiCad review、Allegro topology、BOM refdes matching、Allegro+BOM intake/index report、local datasheet/document match 五块，仍不碰 `.brd`、boardview、placement、routing、PCB geometry、PLM/lifecycle/pricing/availability 或供应商风险。
