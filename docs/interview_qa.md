...
面试时优先讲这条主线：Hardwise 的 headline 是 trust architecture——Refdes Guard + Evidence Ledger + L1 deterministic validators + 结构化工具 + L2/L3 trust 边界——而不是覆盖了多少器件。在这条主线下有两条 public input tracks。KiCad `pic_programmer` 轨用 `review --rules R001,R002,R003,DS001 --report-style component` 证明 registry object -> DS001/L78 reviewed profile token -> guarded report；同一条 L78 路径又用 `ingest-datasheet -> query-datasheet -> ask --vector` 证明 live retrieval 和 agent citation 能跑通——L78 是唯一一条完整 ingest/retrieve/agent-citation smoke，其余 profile 是 reviewed public profile evidence，不说成 live retrieval（边界见 `docs/evidence_chain_audit.md`）。Allegro 轨证明同一套 deterministic validation truth 能扩到 public/synthetic fixture exports 上生成项目级 HTML workbench；当前 README / GitHub Pages 主展示入口是 `design-validator-ui --ai-snapshot` 的 mixed controller Copilot workbench，25 components / 22 validated rows / PASS-WARN-ERROR=5-13-4 / 3 manual rows，其中 22 个 L1 rows 是 9 个 profile-backed targets（U1/U12/U3/U8、D1/D5、Q1/Q2/Q12）+ 13 个 generic passive checks。C3/C4/d3a/d3b/closeout coverage loop 是支撑材料，不是 headline：它证明 ranking 可以持续把 manual rows 推进 L1 deterministic。最新 pressure-test 佐证是——Switch fixture 4010 components / BOM matched 4010 / validated 3794 / manual 216 / PASS-WARN-ERROR 3663-125-6；mainboard fixture 8180 components / BOM matched 7248 / validated 6847 / manual 1333 / PASS-WARN-ERROR 3921-2926-0。generic inductor/ferrite 是轻量 BOM/netlist coverage，diode/MOSFET/BJT family profiles 只在 reviewed public profile + existing validator 下进入 L1；这些是覆盖进展的佐证，不要当主角讲。收口阶段也补了可交付性：Windows/PowerShell recipe 和 `windows-latest` CI workflow 已加入，但 Windows 只能在 Actions 通过后说“已验证”；`74x165_piso_16pin` profile archetype 可以批量生成 `needs_review` 器件档案骨架，但不会自动进入验证，直到人工用公开 datasheet 改成 `ready`。`ask` / `Runner` 通过 structured tools 回答问题；`get_component("U999")` 这类 unknown path 会返回 `found=false`，而不是让模型编造。Eval Pack 只作为 regression / guardrail smoke，不包装成专家准确率 benchmark。
