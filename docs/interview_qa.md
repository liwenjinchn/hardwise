...
**v4.5 input boundary**: V2.5 之后输入模型可以这样讲：native KiCad project 是最高保真入口，有位号、symbol 字段、datasheet path 和可视定位；Allegro schematic netlist 是跨 EDA 的 topology 入口，覆盖 Telesis 单文件和 Capture/Allegro PST 三件套，有 refdes/pin/net 但没有完整 datasheet identity；BOM 是 netlist-only 场景的 component-matching aid，不是 PLM BOM。Hardwise 当前 shipped 的是前两块，BOM matcher 是下一层，不抢进 netlist parser。
