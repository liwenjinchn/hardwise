...
面试时优先讲这条主线：Hardwise 的 headline 是 trust architecture——Refdes Guard + Evidence Ledger + L1 deterministic validators + 结构化工具 + L2/L3 trust 边界——而不是覆盖了多少器件。在这条主线下有两条 public input tracks。KiCad `pic_programmer` 轨用 `review --rules R001,R002,R003,DS001 --report-style component` 证明 registry object -> DS001/L78 reviewed profile token -> guarded report；同一条 L78 路径又用 `ingest-datasheet -> query-datasheet -> ask --vector` 证明 live retrieval 和 agent citation 能跑通——L78 是唯一一条完整 ingest/retrieve/agent-citation smoke，其余 24 个 profile 是 reviewed public profile evidence，不说成 live retrieval（边界见 `docs/evidence_chain_audit.md`）。Allegro 轨证明同一套 deterministic validation truth 能扩到真实大板上生成项目级 HTML workbench；当前 README / GitHub Pages 主展示入口是 `design-validator-ui --ai-snapshot` 的 mixed controller Copilot workbench，25 components / 16 validated rows / PASS-WARN-ERROR=4-9-3 / 9 manual rows，其中 16 个 L1 rows 是 4 个 profile-backed targets + 12 个 generic passive checks。C3/C4/d3a/d3b coverage loop 是支撑材料，不是 headline：它证明 ranking 可以持续把 manual rows 推进 L1 deterministic。在这个支撑框架下的覆盖数字是——早期 4010 颗样本全部 BOM matched，其中 3738 颗已有 L1 deterministic 或 generic passive coverage；最新 D1 mainboard 输入是中文 `.xlsx` BOM + PST 文件夹，8180 schematic components 中 7248 个 BOM matched，6573 个进入 light deterministic coverage，剩余 1607 个保持 manual/profile-gap rows，并生成 195 个 component groups / 75 个 document-index candidates——这些是覆盖进展的佐证，不要当主角讲。最新收口补了两点可交付性：Windows/PowerShell recipe 和 `windows-latest` CI workflow 已加入，但 Windows 只能在 Actions 通过后说“已验证”；`74x165_piso_16pin` profile archetype 可以批量生成 `needs_review` 器件档案骨架，但不会自动进入验证，直到人工用公开 datasheet 改成 `ready`。`ask` / `Runner` 通过 structured tools 回答问题；`get_component("U999")` 这类 unknown path 会返回 `found=false`，而不是让模型编造。Eval Pack 只作为 regression / guardrail smoke，不包装成专家准确率 benchmark。
