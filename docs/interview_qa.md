...
面试时优先讲这条主线：Hardwise 的 headline 是 trust architecture——Refdes Guard + Evidence Ledger + L1 deterministic validators + 结构化工具 + L2/L3 trust 边界——而不是覆盖了多少器件。在这条主线下有两条 public input tracks。KiCad `pic_programmer` 轨用 `review --rules R001,R002,R003,DS001 --report-style component` 证明 registry object -> DS001/L78 reviewed profile token -> guarded report；同一条 L78 路径又用 `ingest-datasheet -> query-datasheet -> ask --vector` 证明 live retrieval 和 agent citation 能跑通——L78 是唯一一条完整 ingest/retrieve/agent-citation smoke，其余 profile 是 reviewed public profile evidence，不说成 live retrieval（边界见 `docs/evidence_chain_audit.md`）。Allegro 轨证明同一套 deterministic validation truth 能扩到真实大板上生成项目级 HTML workbench；当前 README / GitHub Pages 主展示入口是 `design-validator-ui --ai-snapshot` 的 mixed controller Copilot workbench，25 components / 17 validated rows / PASS-WARN-ERROR=5-9-3 / 8 manual rows，其中 17 个 L1 rows 是 4 个 profile-backed targets + 13 个 generic passive checks。C3/C4/d3a/d3b/closeout coverage loop 是支撑材料，不是 headline：它证明 ranking 可以持续把 manual rows 推进 L1 deterministic。最新 pressure-test 佐证是——Switch board 4010 components / BOM matched 4010 / validated 3794 / manual 216 / PASS-WARN-ERROR 3663-125-6；mainboard 8180 components / BOM matched 7248 / validated 6847 / manual 1333 / PASS-WARN-ERROR 3921-2926-0。generic inductor/ferrite 是轻量 BOM/netlist coverage，PE537BA 只在 reviewed public profile + existing MOSFET validator 下进入 WARN；这些是覆盖进展的佐证，不要当主角讲。收口阶段也补了可交付性：Windows/PowerShell recipe 和 `windows-latest` CI workflow 已加入，但 Windows 只能在 Actions 通过后说“已验证”；`74x165_piso_16pin` profile archetype 可以批量生成 `needs_review` 器件档案骨架，但不会自动进入验证，直到人工用公开 datasheet 改成 `ready`。`ask` / `Runner` 通过 structured tools 回答问题；`get_component("U999")` 这类 unknown path 会返回 `found=false`，而不是让模型编造。Eval Pack 只作为 regression / guardrail smoke，不包装成专家准确率 benchmark。
...
**v5.8 (V3.8 EG2132 gate-driver validation)**: V3.8 把 deterministic validation 从 regulator/DCDC 扩到半桥 gate driver。`eg2132_gate_driver` synthetic fixture 里 `U3=EG2132`，VCC 接 `+12V`，HIN/LIN 接 PWM nets，HO/LO 通过栅极电阻到 Q 器件，VS 接半桥开关节点，VB/VS 有 bootstrap 电容，高边 Q 器件还显式接到 `+24V` 母线；故意放了 `D1=MBRA210LT3G` 作为低耐压 bootstrap diode。输出仍是同一个 `ValidationReport`：pin rows 多数是 generic WARN/PASS，真正的 family 判断在 `component_checks`，最终 overall `ERROR` 来自 bootstrap diode rating below required 24 V。后续证据审计把这个 24 V 从 EG2132 datasheet profile 常量改为 validator 从原理图高边母线推导；EG2132 profile 本身只保留官方 PDF 支撑的 pinout、VCC 9-20 V、VCC abs max 25 V、logic-high 2.8 V 和 bootstrap topology。这个例子展示 Hardwise 可以做“拓扑事实 + BOM identity”的稳定检查，但仍不碰死区时序、MOSFET 损耗、PCB 回路、供应链或 PLM。
...
**v5.33 (D3a profile contract store)**: D3a 之后，Q3 的回答更精确：datasheet raw text/page chunks 仍进 Chroma，用来做“给我看第几页证据”；reviewed `DatasheetProfile` contract 进关系库，用 `part_number / alias` 查回，里面是 pin number/name/category/function、limits、recommended topology 和 evidence token。新命令 `store-datasheet-profile data/datasheet_profiles/mpq8626.json --db-path /tmp/hardwise-profiles.db` 会写入 MPQ8626 header + 2 个 aliases + 14 个 ordered pin rows，并读回确认。这个结构库不是 PDF extractor，也不会把 `needs_review` 自动升级成 `ready`；它只是把已经抽取/审过的 contract 从散落 JSON 变成可查询的结构化资产。

**v5.34 (D3b HTML fulltext evidence adapter)**: D3b 补的是 evidence source shape，不是 profile 自动生成。`extract-datasheet-html <path-or-url> --source-name mpq8626.html` 可以把 AllDatasheet-style HTML 全文页抽成 `ChunkRecord` JSONL，保留 `datasheet:mpq8626.html#p1` 这种页码 token；显式传 `--part-ref` 时才入 Chroma。这个适配器解决“公开 HTML 有文字层，但 PDF 直链 guarded 或 image-only”的现实问题，但它不搜索网页、不批量 crawl、不把 HTML 当 PDF cache，也不会自动把 MPQ8626 manual profile 改成 fully provenance-backed ready contract。下一步才是让 agent 在这些 PDF/HTML chunks 上生成 `needs_review` contract draft。
...
**v5.32 (D3a datasheet-contract golden path pivot)**: D3a 没继续把 37 个 IC 候选逐颗补表，而是把方向改成一条可复现链路：`MPN / approved URL -> reviewed direct PDF -> SHA cache -> PDF chunks -> materialized DatasheetProfile contract -> deterministic topology checker`。新增 `fetch-approved-documents`：只读本地 reviewed document-index 的直接 PDF 行，按 SHA-256 存到 cache，并写 JSONL provenance；`candidate/no_row` 等未批准行跳过，网页/非 PDF 字节也跳过。对 `MPQ8626GD-Z` 的公开探测显示 MPS product/documentview 是 HTML/guarded download，Mouser 直链在本机超时，所以 D3a 没把网页硬塞进 evidence store；下一步需要 Datasheets.com 返回的 `datasheetUrl` 或人工 approved 直链。重点讲法：Hardwise 把“找规格书”和“把规格书变成可审计本地证据”拆开了，避免 opaque CDN/product page 伪装成可复现 datasheet。
...
**v5.36 (D3b LN2312LT1G local-symbol alias coverage)**: D3b 解决的是 D2d 留下的 `LN2312LT1G` 真实覆盖 gap：公开 profile 已 ready、MOSFET validator 已存在，但主板 symbol pin id 是 `D/G/S`，不是 datasheet package pin `1/2/3`。新增 `PinProfile.schematic_pin_aliases`，把 `Gate/Source/Drain` 显式映射到 `G/S/D`；统一 resolver 供 candidate matching、pin-level validation、MOSFET checks 和 UI pin consistency 使用。focused Allegro fixture `Q9.G/S/D` 现在验证为 PASS，证据仍是 `datasheet:s-ln2312lt1g.pdf#p1`，没有把本地符号标号伪装成 datasheet pinout。顺手修了 value-text matcher 的边界问题：`MOS LN2312LT1G` 不能再跨 token 误匹配成 `S-LN2312LT1G`，但 `SD103AWS-7-F` 这类带连字符 MPN 仍可匹配。
...
**v5.40 (EG2132 public evidence audit)**: EG2132 审计把“datasheet contract”和“board-level validator policy”拆开了。官方 EGmicro PDF 支撑 pinout、recommended VCC `9-20 V`、VCC abs max `25 V`、HIN/LIN `Vin(H) min 2.8 V` 和 bootstrap diode/capacitor topology；官网产品参数表的 `8-20 V` 被记录为与 PDF 冲突，不覆盖 profile。原先 `bootstrap_diode_min_reverse_voltage=24 V` 没有官方 PDF 直接依据，所以从 `eg2132.json` 移除；`gate_driver_bootstrap` 改为从原理图 switch node 的高边 Q 路径推导所需反压，推不出就 WARN。这样 U3 仍能在 +24V fixture 中稳定报 MBRA210LT3G 低耐压 ERROR，但证据链不再把板级规则伪装成 datasheet 字段。

**v5.41 (public demo packaging closeout)**: 这一版公开 demo 入口从普通静态 validator page 切到 `design-validator-ui --ai-snapshot` 的离线 Copilot workbench。当时 `docs/hardware-demo.html` 是同一条 mixed controller fixture 的生成物：25 components / 16 validated rows / BOM matched=25 / PASS-WARN-ERROR=4-9-3 / 9 manual rows。16 个 L1 rows 要分开讲：U1/U12/U3/U8 是 profile-backed targets，另外 12 个是 generic passive checks，用于 light deterministic coverage，不等同于深度 datasheet review。Copilot panel 内置审计过的 snapshot trace：`run_component_validation` 是 L1 deterministic，L78 evidence-chain smoke 是 L2 grounded，`U999` 这类 unknown refdes 会显示为 `⟨?U999⟩`。Workstream D 后的当前公开数字见 v5.45；不要再把旧的 4 validated / 21 manual HTML 当作当前 demo。

**v5.42 (generic inductor/ferrite Workstream A)**: 通用被动件 coverage 从电容/电阻扩到电感和磁珠，但边界保持很窄：只检查两端连接、BOM/component value 可解析性、封装字段存在性，并在看见显式 current token 时记录 token；不做 buck 拓扑判断、不推断磁珠阻抗编码、不检查纹波/饱和/EMI 充分性。`mixed_controller_power_stage` CLI smoke 从 25 components / 16 validated / 9 manual 移到 17 validated / 8 manual；`motor_sensor_controller` 从 validated=47 / manual=19 移到 validated=55 / manual=11，`recommend-next-family` 不再把已 generic validated 的 inductor/ferrite row 排回队列，只剩 diode 和 unknown。公开 demo HTML 已在 Workstream D 统一重生成。
...
**v5.45 (Workstream D public narrative sync)**: README、GitHub Pages 入口、`docs/demo.md/html`、product intro、JD alignment、recording script 和 `docs/hardware-demo.html` 都同步到当前 mixed controller smoke：25 components / 17 validated rows / BOM matched=25 / PASS-WARN-ERROR=5-9-3 / 8 manual rows。17 个 L1 rows = U1/U12/U3/U8 四个 profile-backed targets + 13 个 generic passive checks，其中新增的 L1 是 `L1=6.8uH` 的 `GENERIC_INDUCTOR` light coverage；它不改变 U12 buck topology ERROR 的 profile-backed 结论。真实 Switch/mainboard 数字只作为 pressure-test summary 链接到 `docs/closeout_pressure_summary.md`，不发布大板 HTML，也不把 coverage 数字包装成整板自动正确性。
