...
> v2.0 — V3.8 adds an EG2132 half-bridge gate-driver family template. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON carries per-pin name/category/function/limits/recommended-topology/evidence facts plus profile-level recommended family limits; `validation/component.py` remains the single validation entry point; `validation/targets.py` parses explicit refdes-to-profile assignments from positional CLI targets or YAML; `validation/profile_candidates.py` suggests candidate refdes-to-profile assignments from BOM MPN/value and local profile `part_number`; `report/validator_ui.py` renders one selected result; `report/validator_multi_ui.py` renders multiple selected `ValidationReport`s in a three-column component index, validation summary, and report-detail artifact. `.brd`, boardview, placement, routing, PCB geometry, live supplier lookup, lifecycle/pricing/availability, hosted app state, automatic validation, and PLM-grade BOM governance remain out of scope.
