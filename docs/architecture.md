...
> v1.6 — V3.4 adds a multi-validation static UI artifact. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON carries per-pin name/category/function/limits/recommended-topology/evidence facts plus profile-level recommended family limits; `validation/component.py` remains the single validation entry point; `report/validator_ui.py` renders one selected result; `report/validator_multi_ui.py` renders multiple selected `ValidationReport`s in one component-index/detail artifact. `.brd`, boardview, placement, routing, PCB geometry, live supplier lookup, lifecycle/pricing/availability, hosted app state, and PLM-grade BOM governance remain out of scope.
...
| `report-validator-ui-batch` in `cli.py` | Generate a local static multi-validation UI | Allegro netlist/PST input, schematic BOM, and one or more `REFDES=profile.json` targets | Single-file HTML with component index, multiple validated component statuses, detail panels, schematic-net panes, scope panes, and per-component markdown downloads | Lets the artifact show PASS and ERROR devices together without creating a hosted app or a second validation truth | Renderer + CLI tests cover a mixed L78 PASS + XL1509 ERROR fixture |
...
`report-validator-ui-batch <netlist_or_pst> <bom> REFDES=profile.json [...]` writes a self-contained HTML validator UI for multiple selected component/profile pairs. It reuses `validate_component_against_profile()` once per target, renders validated component status chips in one artifact, and embeds one markdown download per selected component. It still requires explicit profile assignment per refdes; it is not an auto-profile matcher or a hosted interactive app.
