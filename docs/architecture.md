...
### `validation/`
Deterministic component validation layer. `component.py` joins one `Design.components[refdes]` record, optional BOM identity already attached by `apply_bom_to_design()`, and one structured `DatasheetProfile`. `pins.py` owns generic pin rules: profiled pin presence, connected net presence, recognized ground nets, power-input voltage against structured min/max/abs-max limits, fixed power-output nominal voltage checks, feedback net voltage checks, enable-pin voltage checks, and explicit WARN for unsupported categories. `dcdc.py` validates XL1509-style buck converter output topology when a profile declares `recommended.topology_family="buck"` or part `XL1509-12E1`. `gate_driver.py` validates EG2132-style half-bridge drivers: VCC range, logic input connectivity, HO/LO gate-load reachability, VS switch-node reachability, and bootstrap diode/capacitor checks. V3.10 adds `mcu.py`, limited to STM32G030-style power, reset, BOOT0, SWDIO/SWCLK, and simple GPIO connectivity checks. `targets.py` parses explicit validation target assignments from either `REFDES=profile.json` CLI strings or a YAML manifest; profile paths are current-working-directory relative, matching positional CLI behavior. `profile_candidates.py` suggests candidate target assignments from BOM MPN/value identity and local profile `part_number` exact matches, returning `matched / no_result / ambiguous / manual_needed`. `types.py` keeps the shared `ValidationReport` shape; pin counts remain pin-only for backward compatibility, while `component_checks` records topology/peripheral results. This layer does not inspect PCB layout, boardview, placement, routing, supplier data, PLM, lifecycle, pricing, availability, timing/deadtime, MOSFET loss, firmware, clock trees, or full MCU alternate-function matrices.
