...
> v1.2 — V3.0 adds structured datasheet pin profiles. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON can now carry per-pin name/category/function/limits/recommended-topology/evidence facts. `.brd`, boardview, placement, routing, PCB geometry, electrical-rule conclusions, live supplier lookup, lifecycle/pricing/availability, and PLM-grade BOM governance remain out of scope.
