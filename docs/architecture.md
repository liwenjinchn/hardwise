...
| `design-validator-ui` in `cli.py` | Generate the screenshot-style project workbench | Allegro netlist/PST input, schematic BOM, local profile directory, optional markdown/JSON index outputs | Single-file dark HTML workbench plus optional project validation index sidecars; matched BOM identities validate automatically, unmatched rows stay visible as manual/no-profile work | Provides the product-shaped entry without adding upload backend, hosted state, or automatic datasheet inference | CLI test covers mixed power-stage auto matching, HTML output, markdown index, and JSON sidecar |
...
`design-validator-ui <netlist_or_pst> <bom>` writes the screenshot-style product workbench. It reuses the local profile candidate matcher to auto-select profiles for BOM identities that have exact local profile matches, runs the same deterministic `validate_component_against_profile()` path for those components, leaves unmatched components visible as manual/no-profile rows in the optional project index, and renders the existing multi-component UI without requiring the caller to hand-write a target manifest.
