...
> v1.1 — V2.9 adds a local datasheet/document match layer on top of the Allegro netlist + schematic BOM intake report. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links with `matched / no_result / ambiguous / manual_needed` states. `.brd`, boardview, placement, routing, PCB geometry, electrical-rule conclusions, live supplier lookup, lifecycle/pricing/availability, and PLM-grade BOM governance remain out of scope.
