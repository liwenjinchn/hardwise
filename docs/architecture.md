...
> v1.7 — V3.5 adds a YAML validation targets manifest for `report-validator-ui-batch`. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON carries per-pin name/category/function/limits/recommended-topology/evidence facts plus profile-level recommended family limits; `validation/component.py` remains the single validation entry point; `validation/targets.py` parses explicit refdes-to-profile assignments from positional CLI targets or YAML; `report/validator_ui.py` renders one selected result; `report/validator_multi_ui.py` renders multiple selected `ValidationReport`s in one component-index/detail artifact. `.brd`, boardview, placement, routing, PCB geometry, live supplier lookup, lifecycle/pricing/availability, hosted app state, auto profile matching, and PLM-grade BOM governance remain out of scope.
