...
- `tools.py` — Slice 4 prep plus DR-011 Phase 1 bridge: tool manifest for the tool-use loop. Five tools are wired against parsed registry/store/design facts: `list_components` (relational read with optional value-substring / refdes-prefix filter), `get_component` (single-refdes lookup with a `Found{component}` / `NotFound{refdes, closest_matches}` discriminated-union return — `closest_matches` comes from `difflib.get_close_matches` over `BoardRegistry.refdes_set`, so the agent picks from suggestions, never fabricates), `get_nc_pins` (relational read, optional refdes filter), `search_datasheet` (Chroma vector query, optional `part_ref` filter, returns `DatasheetHit[]` with `page` + `source_pdf` + `part_ref` provenance), and `run_component_validation` (explicit-profile bridge into `validate_component_against_profile`, returning `ValidationRan{overall, counts, checks}` or structured misses such as `ValidationNoProfile` / `ValidationRefdesNotFound{closest_matches}`). Inputs and outputs are Pydantic models; `TOOL_DEFINITIONS` exposes them in Anthropic-SDK `tools=[…]` shape consumed by runner.py. The module is the front-door for mechanisms #1 (Refdes Guard receives `closest_matches` here) and #2 (Evidence Ledger receives source tokens here).
