...
> v1.8 — V3.6 adds a local profile-candidate manifest generator. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON carries per-pin name/category/function/limits/recommended-topology/evidence facts plus profile-level recommended family limits; `validation/component.py` remains the single validation entry point; `validation/targets.py` parses explicit refdes-to-profile assignments from positional CLI targets or YAML; `validation/profile_candidates.py` suggests candidate refdes-to-profile assignments from BOM MPN/value and local profile `part_number`; `report/validator_ui.py` renders one selected result; `report/validator_multi_ui.py` renders multiple selected `ValidationReport`s in one component-index/detail artifact. `.brd`, boardview, placement, routing, PCB geometry, live supplier lookup, lifecycle/pricing/availability, hosted app state, automatic validation, and PLM-grade BOM governance remain out of scope.
...
| `report-validator-ui-batch` in `cli.py` | Generate a local static multi-validation UI | Allegro netlist/PST input, schematic BOM, and either one or more `REFDES=profile.json` targets or `--targets-manifest <yaml>` | Single-file HTML with component index, multiple validated component statuses, detail panels, schematic-net panes, scope panes, and per-component markdown downloads | Lets the artifact show PASS and ERROR devices together without creating a hosted app, auto profile matcher, or a second validation truth | Renderer + CLI tests cover positional targets and a manifest-backed mixed L78 PASS + XL1509 ERROR fixture |
