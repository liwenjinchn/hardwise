...
> v1.3 — V3.1 adds deterministic single-component validation reports. Allegro netlists provide topology; BOM rows provide component identity by refdes join; an optional CSV/TSV document index maps BOM item identity to public datasheet/document links; profile JSON carries per-pin name/category/function/limits/recommended-topology/evidence facts; `validation/component.py` compares one selected component against one structured profile and renders PASS/WARN/ERROR pin rows. `.brd`, boardview, placement, routing, PCB geometry, live supplier lookup, lifecycle/pricing/availability, and PLM-grade BOM governance remain out of scope.
