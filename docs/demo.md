...
90 秒主舞台是 `serve-workbench --fake-ai` 启动的本地 React workbench；
`design-validator-ui --ai-snapshot` 生成同款 SPA 外观的离线单文件快照，作为
GitHub Pages / 无服务环境的备份入口。KiCad `review` / `ask` 命令保留为证据链附录和复现命令，用来证明 registry、L78 检索、Refdes Guard 和 agent tool discipline。
...
| Copilot trace | 展开 Copilot 的 Evidence / Tool trace。 | Copilot 解释 structured tool result，不决定 PASS/WARN/ERROR。 |
...
`serve-workbench --fake-ai` 用确定性 fake client 驱动真实 Runner、五个工具和
Refdes Guard，不需要 API key。`--ai-snapshot` 会把同一份 workbench state、
component detail、prep packet/export 数据和已审计的 Copilot 快照烤进单文件
HTML，不需要 server 或 API key。
...
1. **主舞台是 workbench**：`serve-workbench --fake-ai` 是录屏入口，`design-validator-ui --ai-snapshot` 是同款离线备份，KiCad review/ask 是证据链附录。
...
| File | Handling |
|---|---|
| `docs/hardware-demo.html` | 同款 SPA 外观的离线 Copilot workbench 快照；公开展示备份入口。 |
