...
信任主干是 `registry object -> deterministic validator/rule -> evidence token -> guarded agent/report explanation`。两个输入轨分别展示这条主干的两面：

- **KiCad hero track**：`pic_programmer` 展示 registry-verified refdes、DS001/L78 真实 datasheet token、Refdes Guard 和 agent 工具 discipline。
...
3. **证据是页码级 token**：L78 的 Vin absolute maximum 通过 `datasheet:l78.pdf#p4` 出现在 DS001 finding 和 workbench profile evidence 中。
...
| `docs/interview_narrative.*`, `docs/midpoint_review.*` | 历史收束材料，保留其时间点价值；当前口径以本页、README、`interview_qa.md` 为准。 |
| `docs/PLAN.html` | 阅读版路线图；source of truth 仍是 `docs/PLAN.md`。 |

## 面试时可以这样讲

> Hardwise 的重点不是让模型“独立评审一块板”，而是把模型放进硬件事实边界里。KiCad 轨证明 refdes guard、DS001 datasheet token 和 agent tool discipline；Allegro 轨证明同一套 deterministic validation truth 可以渲染成项目级工作台。两条轨都只用公开数据，所有结论都要落到 registry object 和 evidence token 上。
