...
真实公开 Allegro/PST 样本是第二条 workbench 证据线，作用是证明同一套 deterministic validation truth 能从单器件扩到真实大板，而不是用来比覆盖率。在这个支撑框架下的数字是：4010 颗器件全部 BOM matched，其中 3738 行已有 L1 deterministic 或 generic passive coverage，272 行保持 manual。这里我会特别说明：`GENERIC_CAPACITOR` / `GENERIC_RESISTOR` 是 light deterministic coverage，不等同于深度 datasheet review；深度规则来自 family validator + source-backed profile。`74LV165` shift-register 检查是最好的 demo 点，因为它能看 load/clock fanout 和 Q7-to-DS cascade，不只是查电压。最新补的 `74x165_piso_16pin` profile archetype 则回答“同类型器件怎么办”：它能批量生成 `needs_review` 骨架，但不会自动变成 ready profile。
